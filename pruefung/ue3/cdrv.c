...
static ssize_t mydev_read(struct file *filp, char __user *buff, size_t count, loff_t *offset){
	...
	else if (*offset > dev->buffer_size) {
	...
	if (*offset + count > dev->buffer_size) {
		for (; *offset + count > dev->buffer_size; count -= (*offset + count) - dev->buffer_size) 
			;	
...
static ssize_t mydev_write(struct file *filp, const char __user *buff, size_t count, loff_t *offset) {
	...
	else if (*offset == dev->buffer_size && (filp->f_flags & O_NONBLOCK)) {
	...
	while (count > 0) {
		...
		if (*offset >= dev->buffer_size) {
			if (filp->f_flags & O_NONBLOCK) {
				...
				up(&dev->sync);
				wait_event_interruptible(dev->ioctl_cleared, (dev->current_length != dev->buffer_size));
		...
		to_copy = MIN_VALUE_OF(count, (dev->buffer_size - *offset));
		...
		case IOC_NR_BUFFER_SIZE:
			...
			if (dev->buffer == NULL) {
				...
				if (dev->buffer_size > arg) {
					...
					char *tmp;
					int old = dev->buffer_size;

					tmp = kmalloc(dev->buffer_size, GFP_KERNEL);
					memcpy(tmp, dev->buffer, dev->buffer_size);
					kfree(dev->buffer);
					dev->buffer_size = arg;
					dev->buffer = kmalloc(dev->buffer_size, GFP_KERNEL);
					memcpy(dev->buffer, tmp, old);
					kfree(tmp);
				}
			}	
			pr_devel("cdrv: ioctl: buffer_size set to %ld\n", arg);
			up(&dev->sync);
			return old;
