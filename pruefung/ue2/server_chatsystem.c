...
#define LOG_PATH "/var/log/chatlog002"
...
void *client_handler(void *args)
{
	...
	//printf("User count: %d Max users: %d\n", usercount, max_users);
...
int main(int argc, char **argv)
{
	...
	if (argc > 1) {
		max_users = strtol(argv[1], NULL, 10);
		printf("Max users set to: %d\n", max_users);
	}
	else {
		max_users = 2;
	}
	clients = malloc(max_users * sizeof(chat_client_t));
	init_clients();
