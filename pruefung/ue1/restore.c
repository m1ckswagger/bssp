...
int user_input_original();


int user_input_original() {
	char buf;
	printf("Do you want to restore files with original permissions? (y/n): ");
   ...
   src_fd = open(src_file_path, O_RDWR);
	...
	while(read_inode_and_name(&inode, src_fd, src_file_name) != -1) {
		...
		// Wiederherstellung eines files
			...
			if (mkfifo(src_file_name, inode.st_mode) == -1) {
				perror("creat fifo");
			}
			else {
				printf("%s: named pipe restored\n", src_file_name);
			}
		}
		else {
