...
void backup_node(const char *src_name, const struct stat *target_inode, int target_fd)
{
	...
	if (src_inode.st_size > MAXBKFILESIZE) {
		int result; 
		result = userinput_filesize(); 
		switch (result) {
			case 0: 
				printf("File: %s not archived!\n");
				return;
			case 1: break;
			case -1: exit(4);
		}
...
int userinput_filesize(char *name) {
	char buf;
	printf("Warning! You are about to save a very big file. (%s)\n", name);
	printf("Are you sure you want to save the file (y/n): ");
	if ((buf = fgetc(stdin)) > 0) {
		if (buf == 'y') {
			return 1;
		}
		else if (buf == 'n') {
